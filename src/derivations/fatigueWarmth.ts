...
export async function getBucketFatigue(userId: string, now: Date, pool: Pool = getPool()): Promise<BucketFatigue[]> {
...
export async function getAggregateFatigue(userId: string, now: Date, pool: Pool = getPool()): Promise<number> {
...
export async function getWarmth(userId: string, now: Date, pool: Pool = getPool()): Promise<Warmth> {
  const nowIso = now.toISOString();

  const [generalResult, patternResult] = await Promise.all([
    pool.query<{ general_warmth: number | null }>(
      `
      SELECT SUM(
    ...
    pool.query<{ movement_pattern: string; pattern_warmth: number | null }>(
      `
      SELECT
        x.movement_pattern,
