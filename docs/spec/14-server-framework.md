...
app.get('/api/users/:userId/next', async (c) => {
  ...
  const timezone = c.req.query('timezone')   // required, no stored fallback (Get Next Action, User Profile)
  const now = c.req.query('now') ?? new Date().toISOString()   // optional; real clock only lives here, at the edge (Get Next Action)
  // ...decision pipeline, passed `now` explicitly — nothing downstream calls new Date() or the DB's now()...
...
app.post('/api/users/:userId/log', async (c) => {
  ...
  // ...insert events with source: "onboarding" | "self_directed" (Logging Without a Recommendation)...
  return c.json({ status: 'ok' })
})
...
app.post('/api/users/:userId/readiness', async (c) => {
  const userId = c.req.param('userId')
  const body = await c.req.json()
  const now = body.now ?? new Date().toISOString()   // same pattern as GET /next (Submit Readiness)
...
The four endpoints in [Server API](./05-server-api.md) map onto Hono routes directly, as shown above: `GET /api/users/:userId/next` ([Get Next Action](./05-server-api.md#get-next-action)), `POST /api/users/:userId/results` ([Submit Result](./05-server-api.md#submit-result)), `POST /api/users/:userId/log` ([Logging Without a Recommendation](./05-server-api.md#logging-without-a-recommendation)), `POST /api/users/:userId/readiness` ([Submit Readiness](./05-server-api.md#submit-readiness)). Each handler is where the [decision pipeline](./06-decision-pipeline.md), [result-processing derivations](./07-result-processing.md), and [data-layer queries](./13-data-layer.md) actually get invoked — this spec doesn't prescribe internal handler structure beyond that mapping. `now` is parsed once at the edge, in the handler, per the [Core Principle](./11-core-principle.md) — nothing in the pipeline or query layer should call `new Date()` or rely on the database's own `now()`.
...
Regardless of which entry point is running, application config (`DATABASE_URL` from [Data Layer](./13-data-layer.md), `PORT` for the local/container target) is read from environment variables the same way everywhere. What differs across targets is *which entry point gets built and invoked* — not how that entry point reads its configuration.

## Known Gap: Authentication

Nothing in this spec so far defines how a request establishes *which* `userId` it's allowed to act as — every endpoint takes `userId` as a path parameter with no authentication or authorization layer specified. The data model has been multi-user-ready since [Data Layer](./13-data-layer.md) (every table keyed by `user_id`), but that only matters once requests are actually verified to belong to the user they claim. This needs its own design pass before the API is exposed beyond a trusted single-user client — not addressed here to keep this section scoped to the framework/deployment question it was written to answer.
